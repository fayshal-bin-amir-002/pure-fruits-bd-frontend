...
import { IFruit, UnitType } from "@/types/fruit/fruit";
import { z } from "zod";
...
const UpdateFruitModal: React.FC<AddFruitModalProps> = ({
  fruit,
  isOpen,
  onOpenChange,
}) => {
  ...
  useEffect(() => {
    if (fruit && isOpen) {
      form.reset({
        name: fruit.name || "",
        category: fruit.category?._id || "",
        regular_price: fruit.regular_price || 0,
        offer_price: fruit.offer_price || 0,
        inStock: fruit.inStock ?? true,
        unit: fruit.unit || "kg",
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    try {
      const res = await updateFruit({ id: fruit?._id, data }).unwrap();
      if (res?.success) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[80%] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Update Fruit Item</DialogTitle>
